...
export const openTabActionSchema: ActionSchema = {
  name: 'open_tab',
  description: 'Navigate to URL in the current tab (keeps side panel conversation visible). DO NOT open new tabs.',
  schema: z.object({
    intent: z.string().default('').describe('purpose of this action'),
...
export const scrollToTopActionSchema: ActionSchema = {
  name: 'scroll_to_top',
  description: 'Scroll the document or an element ALL THE WAY to the very top (0%). Use this ONLY when you need to go to the absolute beginning. For incremental scrolling up, use previous_page instead.',
  schema: z.object({
    intent: z.string().default('').describe('purpose of this action'),
    index: z.number().int().nullable().optional().describe('index of the element'),
  }),
...
export const scrollToBottomActionSchema: ActionSchema = {
  ...
  description: 'Scroll the document or an element ALL THE WAY to the very bottom (100%). Use this ONLY when you need to reach the absolute end. For incremental scrolling down, use next_page instead.',
  schema: z.object({
    intent: z.string().default('').describe('purpose of this action'),
    index: z.number().int().nullable().optional().describe('index of the element'),
  }),
...
export const previousPageActionSchema: ActionSchema = {
  name: 'previous_page',
  description:
    'Scroll UP incrementally by one viewport height (like pressing Page Up). Use this for gradual upward scrolling to see more content above. This is the PREFERRED way to scroll up instead of scroll_to_top. If no index is specified, scroll the whole document.',
  schema: z.object({
    intent: z.string().default('').describe('purpose of this action'),
    index: z.number().int().nullable().optional().describe('index of the element'),
  }),
...
export const nextPageActionSchema: ActionSchema = {
  name: 'next_page',
  description:
    'Scroll DOWN incrementally by one viewport height (like pressing Page Down). Use this for gradual downward scrolling to see more content below. This is the PREFERRED way to scroll down instead of scroll_to_bottom. If no index is specified, scroll the whole document.',
  schema: z.object({
    intent: z.string().default('').describe('purpose of this action'),
    index: z.number().int().nullable().optional().describe('index of the element'),
  }),
