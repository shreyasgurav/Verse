    ...
    for (const [i, action] of actions.entries()) {
      ...
      try {
        ...
        if (i > 0 && indexArg !== null) {
          // Use skipWait=true to avoid debugger detachment on SPAs
        ...
        if (error instanceof URLNotAllowedError) {
          // Provide user-friendly message for blocked URLs
          const blockedMessage = `Cannot access this URL (Chrome security restriction). Please try a different approach or URL.`;
          logger.error('URL blocked by security policy:', error.message);
          this.context.emitEvent(Actors.NAVIGATOR, ExecutionState.ACT_FAIL, blockedMessage);
          
          // Add to memory so agent knows to try alternative approach
          const errorMsg = new HumanMessage(`Action error: ${blockedMessage}`);
          logger.info('Adding URL blocked error to memory', errorMsg.content);
          this.context.messageManager.addMessageWithTokens(errorMsg);
          
          // Don't throw - let agent try alternative approach
          errCount++;
          if (errCount > 3) {
            throw new Error('Too many blocked URLs. Task cannot be completed with current approach.');
          }
