...
export class Executor {
  ...
  addFollowUpTask(task: string): void {
    ...
    // CRITICAL: Reset stopped flag so executor can run again after being cancelled
    this.context.stopped = false;
    // Also create a new AbortController for the new task
    this.context.controller = new AbortController();
    
    // CRITICAL FIX: Reset consecutive failures counter for new task
    // This prevents "Max failures reached" error from persisting across tasks
  ...
  async execute(): Promise<void> {
    ...
    if (this.isExecuting) {
      logger.warning('⚠️ Executor is already running, ignoring new execute() call');
      return;
    }
    ...
    // Reset stopped flag at start of execution (safety measure)
    ...
    try {
      ...
      if (isCompleted) {
        ...
        // CRITICAL FIX: Handle max consecutive failures
        ...
        // Track task failure with specific error category
        const maxFailuresError = new MaxFailuresReachedError(t('exec_errors_maxFailuresReached'));
        const errorCategory = analytics.categorizeError(maxFailuresError);
        void analytics.trackTaskFailed(this.context.taskId, errorCategory);
