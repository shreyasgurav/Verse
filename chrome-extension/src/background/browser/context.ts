...
export default class BrowserContext {
  ...
  public async cleanup(): Promise<void> {
    // Remove highlights only if we have a current tab ID
    // Don't call getCurrentPage() as it may trigger fallback logic
    if (this._currentTabId) {
      const currentPage = this._attachedPages.get(this._currentTabId);
      if (currentPage) {
    ...
    // CRITICAL: DON'T reset _currentTabId to null!
    // This BrowserContext is tab-specific and should remember its tab
    // even after cleanup, so follow-up tasks work correctly
    // this._currentTabId = null; // ← REMOVED!
  }
  ...
  public async getCurrentPage(): Promise<Page> {
    ...
    if (!this._currentTabId) {
      // CRITICAL: This should NEVER happen after our fixes!
      // If it does, it indicates a serious bug
      logger.error('❌ CRITICAL: _currentTabId is null! This should never happen!');
      logger.error('This indicates BrowserContext was not properly initialized with a tab ID');
      console.trace(); // Get stack trace for debugging
  ...
  /**
   * Get page for a specific tab WITHOUT switching to it
   * This allows multiple executors to work on different tabs simultaneously
   * Sets _currentTabId so getCurrentPage() returns this tab's page
   */
  ...
  public async navigateTo(url: string): Promise<void> {
    ...
    // Update tab and wait for events (don't force active to allow background execution)
  ...
  public async openTab(url: string): Promise<Page> {
    ...
    // Create the new tab (don't force active to allow background execution)
  ...
  public async getState(useVision = false, cacheClickableElementsHashes = false, skipWait = false): Promise<BrowserState> {
    const currentPage = await this.getCurrentPage();

    const pageState = !currentPage
      ? build_initial_state()
      : await currentPage.getState(useVision, cacheClickableElementsHashes, skipWait);
    const tabInfos = await this.getTabInfos();
    const browserState: BrowserState = {
      ...pageState,
      tabs: tabInfos,
