...
export default class Page {
  ...
  async attachPuppeteer(): Promise<boolean> {
    ...
    // Start keepalive to prevent debugger timeout
  ...
  private async _addAntiDetectionScripts(): Promise<void> {
    ...
    await this._puppeteerPage.evaluateOnNewDocument(`
      ...
      // CRITICAL: Override window.open to force same-tab navigation
      ...
      // CRITICAL: Intercept ALL click events to strip modifiers and target="_blank"
      // This catches Flipkart-style JavaScript that adds Ctrl/Cmd to clicks
      document.addEventListener('click', function(e) {
        const target = e.target;
        
        // Find the closest anchor element (in case clicking child element)
      ...
      // CRITICAL: Also intercept mousedown to catch sites that check modifiers there
  ...
  /**
   * Start keepalive mechanism to prevent debugger timeout
   * Sends a lightweight command periodically to keep the connection alive
   */
  private _startKeepalive(): void {
    // Clear any existing keepalive
    this._stopKeepalive();
    
    // Send a lightweight command every 1 second to keep debugger alive
    this._keepaliveInterval = setInterval(() => {
      if (this._puppeteerPage) {
        // Use a very lightweight operation that doesn't affect the page
        this._puppeteerPage.evaluate('1').catch(() => {
  ...
  async getState(useVision = false, cacheClickableElementsHashes = false, skipWait = false): Promise<PageState> {
    if (!this._validWebPage) {
      // return the initial state
      return build_initial_state(this._tabId);
    }
    // Skip wait for simple DOM reads to prevent debugger detachment on SPAs
    if (!skipWait) {
  ...
  async _updateState(useVision = false, focusElement = -1): Promise<PageState> {
    try {
      ...
      // Try to reattach Puppeteer to recover from context loss
      try {
        logger.info('Attempting to reattach Puppeteer to recover from context loss...');
        await this.detachPuppeteer();
        const reattached = await this.attachPuppeteer();
        
        if (!reattached) {
          logger.error('Failed to reattach Puppeteer - page context may be lost');
          // Return last known good state instead of throwing
          return this._state;
        }
        
        logger.info('Successfully reattached Puppeteer after context loss');
      } catch (reattachError) {
        logger.error('Error during Puppeteer reattachment:', reattachError);
        
        // Last resort: try to get any available page from browser
        if (this._browser) {
          const pages = await this._browser.pages();
          if (pages.length > 0) {
            this._puppeteerPage = pages[0];
            logger.info('Recovered by switching to first available page');
          } else {
            logger.error('Browser closed: no valid pages available');
            return this._state;
          }
        } else {
          return this._state;
  ...
  async sendKeys(keys: string): Promise<void> {
    ...
    try {
      ...
      // Only wait for page load if Enter key (which might trigger navigation)
      // Other keys don't need wait - prevents debugger detachment on SPAs
      if (mainKey.toLowerCase() === 'enter') {
        await this.waitForPageAndFramesLoad();
      }
  ...
  async inputTextElementNode(useVision: boolean, elementNode: DOMElementNode, text: string): Promise<void> {
    if (!this._puppeteerPage) {
      logger.warning('Puppeteer is not connected, attempting to reconnect...');
      // Try to reattach instead of crashing
      if (!(await this.attachPuppeteer())) {
        throw new Error('Puppeteer is not connected and reconnection failed');
      }
    }
    ...
    try {
      ...
      // Use direct value setting for all input types to avoid timing issues with typing
      // This is more reliable than character-by-character typing which can drop characters
      await element.evaluate((el, value) => {
        ...
        // Dispatch all necessary events for form validation and autocomplete
      ...
      // Small delay to allow any autocomplete/suggestions to settle
      await new Promise(resolve => setTimeout(resolve, 100));

      // Skip wait after input - modern SPAs have autocomplete/suggestions that cause constant network activity
      // This was causing debugger detachment. If navigation happens, it will be caught by next getState() call.
      // await this.waitForPageAndFramesLoad();
    } catch (error) {
  ...
  async clickElementNode(useVision: boolean, elementNode: DOMElementNode): Promise<void> {
    ...
    try {
      ...
      // CRITICAL: Force same-tab navigation by removing target="_blank" from links
      // This ensures the side panel conversation stays visible
      await element.evaluate((el) => {
        ...
        // Handle buttons/elements that might have onclick handlers opening new windows
        if (el instanceof HTMLElement) {
      ...
      try {
        ...
        // Force simple left-click without modifiers (no Ctrl, no middle-click)
        // This prevents sites like Flipkart from opening new tabs
        await Promise.race([
          element.click({ 
            button: 'left',
            clickCount: 1,
  ...
  async waitForPageAndFramesLoad(timeoutOverwrite?: number): Promise<void> {
    ...
    // Wait for page load - wrap in try-catch to prevent debugger detachment
    try {
      ...
      // Don't fail on network wait errors - just log and continue
      // This prevents debugger detachment from network monitoring issues
      console.warn('Network wait failed, continuing anyway:', error);
    }
