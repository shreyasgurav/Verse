...
// Key for storing chat session metadata (tab-specific)
const getChatSessionsMetaKey = (tabId?: number) => 
  tabId !== undefined ? `chat_sessions_meta_tab_${tabId}` : 'chat_sessions_meta';

// Create storage for session metadata (tab-specific)
const getChatSessionsMetaStorage = (tabId?: number) => {
  return createStorage<ChatSessionMetadata[]>(getChatSessionsMetaKey(tabId), [], {
    storageEnum: StorageEnum.Local,
    liveUpdate: true,
  });
};

// Helper function to get storage key for a specific session's messages (tab-specific)
const getSessionMessagesKey = (sessionId: string, tabId?: number) => 
  tabId !== undefined ? `chat_messages_tab_${tabId}_${sessionId}` : `chat_messages_${sessionId}`;

// Helper function to create storage for a specific session's messages (tab-specific)
const getSessionMessagesStorage = (sessionId: string, tabId?: number) => {
  return createStorage<ChatMessage[]>(getSessionMessagesKey(sessionId, tabId), [], {
    storageEnum: StorageEnum.Local,
    liveUpdate: true,
  });
};

// Helper function to get storage key for a specific session's agent state history (tab-specific)
const getSessionAgentStepHistoryKey = (sessionId: string, tabId?: number) => 
  tabId !== undefined ? `chat_agent_step_tab_${tabId}_${sessionId}` : `chat_agent_step_${sessionId}`;

// Helper function to get storage for a specific session's agent state history (tab-specific)
const getSessionAgentStepHistoryStorage = (sessionId: string, tabId?: number) => {
  return createStorage<ChatAgentStepHistory>(
    getSessionAgentStepHistoryKey(sessionId, tabId),
    {
      task: '',
      history: '',
      timestamp: 0,
 ...
 * @param tabId Optional tab ID for tab-specific storage
 */
export function createChatHistoryStorage(tabId?: number): ChatHistoryStorage {
  const chatSessionsMetaStorage = getChatSessionsMetaStorage(tabId);
  ...
  return {
    ...
    clearAllSessions: async (): Promise<void> => {
      ...
      for (const sessionMeta of sessionsMeta) {
        const messagesStorage = getSessionMessagesStorage(sessionMeta.id, tabId);
    ...
    getSession: async (sessionId: string): Promise<ChatSession | null> => {
      ...
      const messagesStorage = getSessionMessagesStorage(sessionId, tabId);
    ...
    createSession: async (title: string): Promise<ChatSession> => {
      ...
      const messagesStorage = getSessionMessagesStorage(newSessionId, tabId);
      await messagesStorage.set([]);
    ...
    storeAgentStepHistory: async (sessionId: string, task: string, history: string): Promise<void> => {
      ...
      const agentStepHistoryStorage = getSessionAgentStepHistoryStorage(sessionId, tabId);
